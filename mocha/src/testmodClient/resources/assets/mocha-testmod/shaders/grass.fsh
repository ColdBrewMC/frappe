#version 330

// Source - https://stackoverflow.com/a/17897228
// Posted by sam hocevar, modified by community. See post 'Timeline' for change history
// Retrieved 2026-06-07, License - CC BY-SA 4.0

// All components are in the range [0…1], including hue.
vec3 rgb2hsv(vec3 c)
{
	vec4 K = vec4(0.0, -1.0 / 3.0, 2.0 / 3.0, -1.0);
	vec4 p = mix(vec4(c.bg, K.wz), vec4(c.gb, K.xy), step(c.b, c.g));
	vec4 q = mix(vec4(p.xyw, c.r), vec4(c.r, p.yzx), step(p.x, c.r));

	float d = q.x - min(q.w, q.y);
	float e = 1.0e-10;
	return vec3(abs(q.z + (q.w - q.y) / (6.0 * d + e)), d / (q.x + e), q.x);
}

// Source - https://stackoverflow.com/a/17897228
// Posted by sam hocevar, modified by community. See post 'Timeline' for change history
// Retrieved 2026-06-07, License - CC BY-SA 4.0

// All components are in the range [0…1], including hue.
vec3 hsv2rgb(vec3 c)
{
	vec4 K = vec4(1.0, 2.0 / 3.0, 1.0 / 3.0, 3.0);
	vec3 p = abs(fract(c.xxx + K.xyz) * 6.0 - K.www);
	return c.z * mix(K.xxx, clamp(p - K.xxx, 0.0, 1.0), c.y);
}

void frp_inputFragment() {
	float ao = 1.0 - ftm_vertAo;
//	const vec3 aoColor = vec3(0.09113, 0.3361, 0.05403);
	// make a generic color out of the current fragment
	vec3 hsv = rgb2hsv(frp_fragColor.rgb);
	hsv.z = min(hsv.z * 1.5, 0.85);
	hsv.y = min(hsv.y * 1.5, 0.85);
	vec3 aoColor = hsv2rgb(hsv);
	vec3 color = vec3(0.0);
	const float maxcolor = 0.25;
	color += (frp_quadMaterialId == FRP_MATERIAL_ID) ? min(ao, maxcolor) * aoColor : vec3(0.0);
//	color *= color;
	frp_fragColor.rgb += color;
	//	ftm_vertAo *= (frp_quadMaterialId == FRP_MATERIAL_ID) ? 0.8 : 1.0;
}
