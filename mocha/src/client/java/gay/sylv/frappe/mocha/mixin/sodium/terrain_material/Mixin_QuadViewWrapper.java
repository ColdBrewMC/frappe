...
public abstract class Mixin_QuadViewWrapper implements QV_ExtTerrainMaterial {
	...
	public TerrainMaterial frappe$terrainMaterial() {
		return MochaIndigoEncodingFormat.terrainMaterial(this.quad.data[this.quad.baseIndex + HEADER_MOCHA_BITS]);
	}

	@Override
	public float frappe$u(int vertexIndex) {
		return Float.intBitsToFloat(this.quad.data[this.quad.baseIndex + HEADER_MOCHA_BITS + FRAPPE_U_0 + vertexIndex * 2]);
	}
