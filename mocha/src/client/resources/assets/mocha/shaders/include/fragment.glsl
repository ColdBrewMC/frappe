...
#if false
in uint v_FrappeMaterialId;
#endif

#ifdef _FRAPPE_SIMPLE_MATERIAL
vec4 _frappe_simple_pre_fragment(vec4 color) {
	#ifdef _FRAPPE_SIMPLE_PRE_FRAGMENT
	color = _frappe_simple_pre_fragment__FRAPPE_MATERIAL_ID(color, float(v_FrappeMaterialId == _FRAPPE_MATERIAL_IDu));
	#endif
	return color;
}
#endif
...
vec4 _frappe_pre_fragment(vec4 color) {
	#ifdef _FRAPPE_PRE_FRAGMENT
	color = _frappe_pre_fragment__FRAPPE_MATERIAL_ID(color, float(v_FrappeMaterialId == _FRAPPE_MATERIAL_IDu));
	#endif
	return color;
}
#endif
